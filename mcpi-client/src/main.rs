...
use mcpi_common::{
    ...
    Provider, Referral, CapabilityDescription, InitializeParams, ListResourcesResult, ListToolsResult,
};
...
async fn run_mcp_http_client( mcp_url: String, specific_plugin: Option<String>, _discovery_resp: DiscoveryResponse ) -> Result<(), BoxedError> {
    let http_client = ReqwestClient::new(); let mut session_id: Option<String> = None; let mut headers = HeaderMap::new(); headers.insert(CONTENT_TYPE, HeaderValue::from_static("application/json"));
    println!("\nEstablishing SSE connection via GET {}...", mcp_url);
    let get_response = http_client.get(&mcp_url).header(ACCEPT, "text/event-stream").send().await?; if !get_response.status().is_success() { return Err(format!("GET /mcp failed status: {}", get_response.status()).into()); } if let Some(ct) = get_response.headers().get(CONTENT_TYPE) { if !ct.to_str()?.starts_with("text/event-stream") { return Err(format!("Expected text/event-stream, got: {:?}", ct).into()); } } else { return Err("Missing Content-Type on GET /mcp response".into()); } if let Some(sid_value) = get_response.headers().get(MCP_SESSION_ID_HEADER) { let sid = sid_value.to_str()?.to_string(); println!("Obtained MCP session ID: {}", sid); session_id = Some(sid.clone()); headers.insert(MCP_SESSION_ID_HEADER, HeaderValue::from_str(&sid)?); } else { println!("Warning: Server did not provide mcp-session-id header."); } println!("SSE stream ostensibly connected (event processing not implemented yet).");
    println!("\nSending initialize request via POST {}...", mcp_url); let init_params = InitializeParams { client_info: mcpi_common::Implementation { name: "MCP Test Client".to_string(), version: "0.1.0".to_string() }, protocol_version: LATEST_MCP_VERSION.to_string(), capabilities: Default::default() }; let init_request = MCPRequest { jsonrpc: "2.0".to_string(), id: json!(1), method: "initialize".to_string(), params: Some(serde_json::to_value(init_params)?) }; let init_req_str = serde_json::to_string(&init_request)?; let post_response = http_client.post(&mcp_url).headers(headers.clone()).body(init_req_str).send().await?; if !post_response.status().is_success() { return Err(format!("Initialize POST failed status: {}", post_response.status()).into()); } let init_resp_body = post_response.json::<MCPResponse>().await?; if let Some(err) = init_resp_body.error { println!("Init error: {} ({})", err.message, err.code); return Err("Init failed".into()); } if let Some(res) = init_resp_body.result { let init_res: InitializeResult = serde_json::from_value(res)?; println!("\nMCP initialized: Server: {} v{}, Proto: v{}", init_res.server_info.name, init_res.server_info.version, init_res.protocol_version); if let Some(inst) = init_res.instructions { println!("  Instructions: {}", inst); } } else { return Err("Invalid init response".into()); }
    println!("\nListing Resources via POST {}...", mcp_url); let list_res_req = MCPRequest { jsonrpc: "2.0".to_string(), id: json!(2), method: "resources/list".to_string(), params: None }; let list_res_str = serde_json::to_string(&list_res_req)?; let list_res_resp = http_client.post(&mcp_url).headers(headers.clone()).body(list_res_str).send().await?.json::<MCPResponse>().await?; if let Some(e) = list_res_resp.error { println!("Err list res: {} ({})", e.message, e.code); } else if let Some(r) = list_res_resp.result { match serde_json::from_value::<ListResourcesResult>(r) { Ok(list_result) => { println!("\nAvailable MCP resources:"); if list_result.resources.is_empty() { println!("  (No resources)"); } else { for item in list_result.resources { println!("  - {} ({})", item.name, item.uri); if let Some(d)=item.description{println!("    Desc: {}",d);} } } if list_result.next_cursor.is_some() { println!("  (More resources available...)"); } } Err(e) => println!("Warn: Failed to parse ListResourcesResult: {}", e), } } else { println!("Warn: Invalid list res resp (no result/error)"); }
    ...
    println!("\n(MCP HTTP Client finished - Requires full SSE stream handling & POST->SSE response handling)"); Ok(())
}
...
async fn call_tool_http( client: &ReqwestClient, mcp_url: &str, session_id: Option<&str>, name: &str, operation: &str, arguments: Value ) -> Result<(), BoxedError> {
    println!("\nTesting {} with {} operation (HTTP)", name, operation);
    let req=MCPRequest{jsonrpc:"2.0".to_string(),id:json!(format!("{}-{}-{}",name,operation,rand::thread_rng().gen::<u16>())),method:"tools/call".to_string(),params:Some(json!({"name":name,"arguments":arguments}))};
    println!("Request Params: {}", serde_json::to_string_pretty(req.params.as_ref().unwrap()).unwrap_or_default());
    let req_str=serde_json::to_string(&req).map_err(|e|format!("Serialize tool call req err: {}",e))?;
    ...
    let response = client.post(mcp_url).headers(headers).body(req_str).send().await.map_err(|e| format!("POST /mcp tool call failed: {}", e))?;
    ...
    if content_type.starts_with("application/json") {
        let resp_body = response.json::<MCPResponse>().await.map_err(|e| format!("Parse tool call resp err: {}", e))?;
         match resp_body {
