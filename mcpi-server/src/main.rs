...
use axum::{
    ...
    response::{sse::{Event, KeepAlive, Sse}, IntoResponse, Response}, // Added sse types
    routing::{get, post, delete},
    Router, Json,
    // Removed BoxError
    // Correctly import HeaderName/Value from axum's http re-export
...
use mcpi_common::{ // Group common imports
    ...
    // Removed unused capability structs
    ResourceContentUnion, TextResourceContents,
    LATEST_MCP_VERSION, // Import constant
};
...
use std::{
    ...
    time::{Instant, Duration}, // Added Duration
    fs,
    error::Error,
    convert::Infallible, // For SSE stream error type
};
use tokio::net::TcpListener;
use tokio::sync::{mpsc, RwLock}; // Added mpsc
...
use url::Url;
use rand::Rng;
// Import futures stream trait and tokio_stream adapter/map
use futures::Stream;
use tokio_stream::wrappers::ReceiverStream;
use tokio_stream::StreamExt; // Needed for .map() on the stream
...
// Removed PROTOCOL_VERSION_PLACEHOLDER, using LATEST_MCP_VERSION now
...
async fn handle_streamable_get(
    ...
    let provided_session_id = headers.get(&MCP_SESSION_ID).and_then(|hv| hv.to_str().ok()).map(str::to_string);

    let session_id: String;
    let receiver: mpsc::Receiver<Event>;
    ...
    match provided_session_id {
        Some(existing_id) if sessions.contains_key(&existing_id) => {
            info!("SSE stream resuming session: {}", existing_id);
            session_id = existing_id;
            let (sender, new_receiver) = mpsc::channel(10); // Buffer size 10
            sessions.insert(session_id.clone(), HttpSessionInfo { sender });
    ...
    // Convert the receiver into a stream compatible with Sse
    let stream = ReceiverStream::new(receiver)
        .map(Ok::<_, Infallible>); // Map Event to Result<Event, Infallible>
...
// POST /mcp - Handles incoming MCP requests over HTTP
...
fn handle_initialize(_request: &MCPRequest, registry: &Arc<PluginRegistry>, provider_info: &Arc<Value>) -> String {
    let caps=ServerCapabilities{ resources:Some(ResourcesCapability{list_changed:true,subscribe:true}), tools:Some(ToolsCapability{list_changed:true}), prompts:None, logging:None, completions: None, experimental: None }; let name=provider_info.get("name").and_then(|v|v.as_str()).unwrap_or("").to_string(); let desc=provider_info.get("description").and_then(|v|v.as_str()).unwrap_or("").to_string(); let _names=registry.get_all_plugins().iter().map(|p|p.name()).collect::<Vec<_>>();
    let result = InitializeResult { server_info: mcpi_common::Implementation { name, version: MCPI_VERSION.to_string() }, protocol_version: LATEST_MCP_VERSION.to_string(), capabilities: caps, instructions: Some(format!("Provider: {}", desc)), _meta: None, };
...
fn handle_list_resources(_request: &MCPRequest, registry: &Arc<PluginRegistry>, provider_info: &Arc<Value>) -> String {
    let domain=provider_info.get("domain").and_then(|d|d.as_str()).unwrap_or("example.com"); let resources=registry.get_all_plugins().iter().flat_map(|p|p.get_resources().into_iter().map(|(n,s,d)|Resource{ name:n, description:d, uri:format!("mcpi://{}/resources/{}/{}",domain,p.name(),s), mime_type:Some("application/json".into()), annotations: None })).collect::<Vec<_>>();
    let result = ListResourcesResult { resources, next_cursor: None, _meta: None, }; json!({"jsonrpc":"2.0","id":_request.id, "result": result }).to_string()
}
...
fn handle_list_tools(_request: &MCPRequest, registry: &Arc<PluginRegistry>) -> String {
    ...
    let result = ListToolsResult { tools, next_cursor: None, _meta: None, }; json!({"jsonrpc":"2.0","id":_request.id, "result": result }).to_string()
}
